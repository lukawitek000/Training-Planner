...
class ImageStorageTest {
    ...
    @Test
    fun `saves the image and read its reference`() =
        runBlocking {
    ...
    @Test
    fun `read image reference for not existing image id returns null`() =
        runBlocking {
    ...
    @Test
    fun `read deleted image throws exception`() =
        runBlocking {
    ...
    @Test
    fun `delete image for one owner`() =
        runBlocking {
            val ownerId = "abc123".toUUID()
    ...
    @Test
    fun `read deleted image for all owners throws exception`() =
        runBlocking {
    ...
    @Test
    fun `delete image that does not exist throws exception`() =
        runBlocking {
            val ownerId = "abc123".toUUID()
            assertFailsWith<ImageNotFoundException> {
    ...
    @Test
    fun `update saved image for the owner`() =
        runBlocking {
            val imageConfiguration = givenImageConfiguration()
            val imageReference = imageStorage.saveImage(imageConfiguration)
    ...
    @Test
    @Suppress("LongMethod")
    fun `update image for all owners`() =
        runBlocking {
    ...
    @Test
    fun `update image for only 1 of 2 owners`() =
        runBlocking {
            val owners = listOf("abc123".toUUID(), "123abc".toUUID())
            val imageConfiguration1 = givenImageConfiguration(ownerId = owners[0])
            val imageConfiguration2 = givenImageConfiguration(ownerId = owners[1])
    ...
    @Test
    fun `update method saves image if it did not exist`() =
        runBlocking {
            val imageConfiguration = givenImageConfiguration()
    ...
    ): ImageConfiguration = ImageConfiguration(data, ownerId)
