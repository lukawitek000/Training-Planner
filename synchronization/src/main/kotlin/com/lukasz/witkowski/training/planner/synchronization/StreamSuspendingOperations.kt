...
suspend fun OutputStream.closeSuspending() =
    withContext(Dispatchers.IO) {
        close()
    }

suspend fun OutputStream.writeSuspending(byteArray: ByteArray) =
    withContext(Dispatchers.IO) {
        write(byteArray)
        flush()
    }

suspend fun OutputStream.writeByteSuspending(number: Int) =
    withContext(Dispatchers.IO) {
        write(number)
    }

suspend fun OutputStream.writeIntSuspending(number: Int) =
    withContext(Dispatchers.IO) {
        val byteArray = number.toByteArray()
        writeSuspending(byteArray)
    }

suspend fun InputStream.closeSuspending() =
    withContext(Dispatchers.IO) {
        close()
    }

suspend fun InputStream.readByteSuspending() =
    withContext(Dispatchers.IO) {
        read()
    }

suspend fun InputStream.readByteSuspending(buffer: ByteArray): Int =
    withContext(Dispatchers.IO) {
        read(buffer)
    }

suspend fun InputStream.readIntSuspending(): Int =
    withContext(Dispatchers.IO) {
